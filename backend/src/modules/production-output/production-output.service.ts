  ...
  async create(user: AuthUser, dto: CreateOutputDto) {
    ...
    const output = await this.prisma.productionOutput.create({
      data: {
        ...
        // Packing stage — hardener/thinner counts + their own size/unit (kg/L never blended).
        hardenerCount: dto.hardenerCount ?? 0,
        hardenerSize: dto.hardenerSize ?? null,
        hardenerUnit: dto.hardenerUnit ?? null,
        thinnerCount: dto.thinnerCount ?? 0,
        thinnerSize: dto.thinnerSize ?? null,
        thinnerUnit: dto.thinnerUnit ?? null,
  ...
  async update(user: AuthUser, id: string, dto: UpdateOutputDto) {
    ...
    const output = await this.prisma.productionOutput.update({
      ...
      data: {
        ...
        hardenerCount: dto.hardenerCount,
        hardenerSize: dto.hardenerSize,
        hardenerUnit: dto.hardenerUnit,
        thinnerCount: dto.thinnerCount,
        thinnerSize: dto.thinnerSize,
        thinnerUnit: dto.thinnerUnit,
      },
      include: outputInclude,
    });
    await this.audit.log({
